...
1. To validate the contents of the emails received (json string).
...
|reply-id-algorithm|The algorithm used by the server at the moment the reply is sent.|
|from         |The email address of the sender.|
|sender-fingerprint|A hash string generated by the sender using the sender's secret code.|
|to           |An array of strings each one containing an email address. Cannot be empty.|
|sent-time    |A string representing a timestamp in ISO 8601 in UTC timezone (UTC always).|
...
```
[reply to message-id=2e87284d245... from lisa@bar.com]
    | 
    +---> foo.com --> DB -+--> notify: steve@foo.com 
    |                     |
    |                     +--> sales-team@foo.com ---> notify: susan@foo.com, mark@foo.com, lauren@foo.com, ...
...
## Identifying emails and replies
...
An HTTP request is sent to the original domain including the original sender's address and the `sender-fingerprint` and the `message-id` values as follows:
